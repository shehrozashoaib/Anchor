...
- [`system_debug_cli.py`](./system_debug_cli.py): terminal launcher and startup flow
- [`system_llm_agent.py`](./system_llm_agent.py): main debugging loop and reasoning logic
- [`system_tools.py`](./system_tools.py): script execution, terminal commands, file reads/writes, output capture
...
- prepares the agent environment at `.venvs/agent`
- prepares the user runtime at `.venvs/user`
...
- regenerates helper scripts for the local checkout:
  - [`agent_env.sh`](./agent_env.sh)
  - [`run_agent.sh`](./run_agent.sh)
  - [`run_user_code.sh`](./run_user_code.sh)
...
Useful logs are created after setup/build in this folder:
...
Using the environment variable is preferred over editing [`run_agent.sh`](./run_agent.sh), because setup can regenerate the launcher script.
...
- Hidden setup logs live under `./.setup_logs` after setup/build runs.
- This file documents the current `v5` release.
