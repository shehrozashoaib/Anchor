...
def get_user_approval_for_action(action: AgentAction, state: Dict[str, Any]) -> tuple[bool, str]:
    """
    Interactive user gate for mutating actions after an error is observed.
    Returns (approved, feedback_message_for_llm_if_rejected).
    """
    if not state.get("awaiting_user_feedback", False):
        return True, ""

    if action.action_type not in {"WriteFileInput", "ExecuteTerminalCommand", "ConfigInput"}:
        return True, ""
    ...
    choice = safe_input("Select option [y/n/p]: ").strip().lower() or "y"
