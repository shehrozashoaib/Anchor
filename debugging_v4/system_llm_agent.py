...
def normalize_action_payload(obj: Dict[str, Any], state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if action_type == "ReadFileInput":
        ...
        if lower_command in {"pip install torch", "python -m pip install torch", "pip3 install torch"}:
            ...
            if recommended_command:
                action["command"] = recommended_command
        else:
            last_error = state.get("full_error_history", [])[-1] if state.get("full_error_history") else {}
...
def review_latest_run_output(state: Dict[str, Any], label: str = "script run") -> None:
    ...
    print(f"\n📋 Output Review available for {label} (exit={exitcode})")
    print(f"   stdout chars: {len(stdout)} | stderr chars: {len(stderr)}")
...
def get_user_approval_for_action(action: AgentAction, state: Dict[str, Any]) -> tuple[bool, str]:
    ...
    """
    if not state.get("awaiting_user_feedback", False):
        return True, ""
    ...
    print("-" * 80)
    print(f"Proposed action: {_summarize_action_for_approval(action)}")
    if action.action_type == "WriteFileInput":
    ...
    choice = safe_input("Select option [y/n/p/r]: ").strip().lower() or "y"
...
def search_online_for_solution(
    ...
    if error_type in {"ImportError", "ModuleNotFoundError"}:
        missing_import_name = (error_message or "").strip().strip("'\"").lower()
...
def _classify_environment_compatibility_issue(error_info: Dict[str, Any], stderr: str) -> Dict[str, Any]:
    ...
    if error_type not in {"ImportError", "ModuleNotFoundError", "AttributeError", "TypeError"}:
...
def _build_env_diagnosis_command(package_names: List[str]) -> str:
    packages = [pkg for pkg in package_names if pkg][:6]
    if packages:
...
def _build_env_resolution_guidance(classification: Dict[str, Any], state: Dict[str, Any]) -> str:
    ...
    diag_command = _build_env_diagnosis_command(packages)
...
def run_llm_debug_agent(
    ...
    if use_mocking_strategy is None:
        ...
        print(f"\n   ✅ Strategy selected by caller: {selected}")
    ...
    for step in range(max_steps):
        ...
        print(f"   🤖 Next action: {_summarize_action_for_approval(action)}")
            ...
            # === FAILURE PATH ===
                ...
                if can_run_rag:
                    ...
                    if env_issue.get("is_env_compatibility"):
                        print("\n🧪 ENVIRONMENT COMPATIBILITY FAILURE DETECTED")
                        print("   Treating this as a third-party package/version mismatch, not a source edit problem.")
        ...
        if action.action_type == "ReadFileInput":
            ...
            if is_library:
                ...
                if state.get("env_resolution_active"):
                    diag_command = state.get("env_resolution_command") or _build_env_diagnosis_command(state.get("env_resolution_packages", []))
                    state["current_input"] = (
