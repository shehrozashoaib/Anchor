...
def _lower(value: Any) -> str:
    return _lower_text(value).lower()
                ...
                for class_name, class_config in structure.items():
                    if isinstance(class_config, dict):
                        for attr, attr_info in class_config.items():
                            if (
                                attr == inp.param_name
                                or _lower(attr) == _lower(inp.param_name)
                            ):
                                inp.source = "config"
                                inp.config_path = (
        ...
        def search_nested(d, prefix=""):
            ...
            for key, value in d.items():
                ...
                for target in target_params:
                    if _lower(key) == _lower(target) or _lower(target) in _lower(key):
                        found[target] = {
                            "value": value,
                            "path": full_key,
...
'''
        ...
        needs_model = any(
            inp["name"] == "model" or "model" in _lower(inp.get("type") or "")
            for inp in func_spec.get("inputs", [])
        )
        ...
        needs_loaders = any(
            "loader" in _lower(inp["name"]) or "data" in _lower(inp["name"])
            for inp in func_spec.get("inputs", [])
        )
        ...
        if needs_loaders or "dataloader" in func_spec.get("expensive_ops", []):
            ...
            num_samples = next(
                (
                    p["minimal_value"]
                    for p in critical_params.get("critical_minimal", [])
                    if "train" in _lower(p["param"]) or "sample" in _lower(p["param"])
                ),
                5,
            )
        ...
        for inp in func_spec.get("inputs", []):
            ...
            elif "loader" in _lower(param_name):
...
class EnhancedParameterResolver(SmartParameterResolver):
    ...
    def _determine_minimal_value(self, attr_name: str, attr_info: Dict) -> Any:
        ...
        if any(kw in _lower(attr_name) for kw in ["epoch", "n_epoch", "num_epoch"]):
            return 1

        elif any(kw in _lower(attr_name) for kw in ["batch_size", "batch"]):
            return 2

        elif any(
            kw in _lower(attr_name)
            ...
            kw in _lower(attr_name) for kw in ["n_test", "test_size", "num_test"]
        ):
            return 2

        elif any(kw in _lower(attr_name) for kw in ["num_worker", "n_worker"]):
            return 0

        elif any(kw in _lower(attr_name) for kw in ["max_neighbor", "neighbor"]):
            return 5

        elif any(kw in _lower(attr_name) for kw in ["hidden", "dim", "embed"]):
            if isinstance(default, int) and default > 32:
                return 8  # Minimal hidden dimension
            return default

        elif any(kw in _lower(attr_name) for kw in ["layer", "n_layer", "num_layer"]):
            return 1

        elif "ratio" in _lower(attr_name):
            # Keep ratios as-is
            return default

        elif _lower(attr_name) in ["learning_rate", "lr"]:
        ...
        # For paths, keep default
            ...
            "path" in _lower(attr_name)
            or "dir" in _lower(attr_name)
            or "file" in _lower(attr_name)
        ):
            return default
