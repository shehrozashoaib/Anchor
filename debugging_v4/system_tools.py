...
def _resolve_user_python() -> str:
    """Return the preferred Python interpreter for running user code and its installs."""
    user_env_dir = os.environ.get("USER_ENV_DIR", "/workspace/.venvs/user")
    candidate = os.path.join(user_env_dir, "bin", "python")
    return candidate if os.path.exists(candidate) else sys.executable


def _build_python_subprocess_env(python_bin: str) -> Dict[str, str]:
    env = os.environ.copy()
    python_dir = os.path.dirname(python_bin)
    env["PATH"] = python_dir + os.pathsep + env.get("PATH", "")
    env["VIRTUAL_ENV"] = os.path.dirname(python_dir)
    return env


def _resolve_conda_executable(env: Dict[str, str]) -> str | None:
    candidates = [
        env.get("CONDA_EXE"),
        shutil.which("conda", path=env.get("PATH", "")),
        "/opt/miniforge3/bin/conda",
        "/opt/miniforge3/condabin/conda",
    ]
...
def _detect_file_type_fallback(file_path: str) -> str:
    """Best-effort file type detection without libmagic."""
...
def _safe_magic_file_type(file_path: str) -> str:
    """Use python-magic when available; otherwise fall back to extension/text heuristics."""
    try:
        import magic  # type: ignore[import-not-found]

        return magic.Magic().from_file(file_path)
    except Exception:
        return _detect_file_type_fallback(file_path)
...
def list_files() -> Dict[str, str]:
    ...
    for f in os.listdir("."):
        file_list[f] = _safe_magic_file_type(f)
...
def terminal_command_executor(command: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "command": command,
            "stdout": "",
            "stderror": f"Command parsing failed: {e}",
            "exit_code": 1,
        }
    ...
    python_bin = _resolve_user_python()
    env = _build_python_subprocess_env(python_bin)

    executable = parts[0]
    normalized = os.path.basename(executable)
    if normalized in {"pip", "pip3"}:
        parts = [python_bin, "-m", "pip"] + parts[1:]
    elif normalized in {"python", "python3"}:
        parts[0] = python_bin
    elif normalized == "conda":
        conda_exe = _resolve_conda_executable(env)
        if conda_exe:
            parts[0] = conda_exe
            if len(parts) > 1 and parts[1] == "install":
                has_target = any(flag in parts for flag in ["-n", "--name", "-p", "--prefix"])
                if not has_target:
                    user_env_dir = os.environ.get("USER_ENV_DIR", "/workspace/.venvs/user")
                    parts = [parts[0], parts[1], "-p", user_env_dir] + parts[2:]
        else:
            return {
                "command": parts,
                "stdout": "",
                "stderror": "Conda executable not found in the current runtime environment.",
                "exit_code": 1,
            }
    ...
    try:
        ...
        output_dict["stdout"] = ""
        output_dict["stderror"] = str(e)
        output_dict["exit_code"] = 1
...
def read_file(
    ...
    try:
        ...
        if file_type is None:
            file_type = _safe_magic_file_type(file_path)
...
def run_script(script_path: str, args: List[str] | None = None) -> Dict[str, Any]:
    ...
    python_bin = _resolve_user_python()
    command = [python_bin, script_path] + args
